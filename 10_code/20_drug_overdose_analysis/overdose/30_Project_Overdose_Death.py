...
df11 = pd.read_csv("Underlying Cause of Death, 2005.txt",  delimiter="\t")
df12 = pd.read_csv("Underlying Cause of Death, 2004.txt",  delimiter="\t")
...
df_final['Policy_State'] = df_final['Policy_State'].astype('category')
...
fig_FL=(ggplot(aes(x='Year', y='Death_Rate', group = 'State', color = 'State') , data = df_FL) +
        geom_point(color = 'grey', alpha = 0.2) +geom_smooth(method='lm', data=df_FL[df_FL['Year']<2010], color='blue') +
        geom_smooth(method='lm', data=df_FL[df_FL['Year']>=2010], color='red') + geom_vline(xintercept=2010, linetype = "dotted") + 
        xlim(2004, 2015)+
        labs(y='Death Rate by Overdose', title="Pre-Post in FL")
       )
...
#TX prepost
fig_TX=(ggplot(aes(x='Year', y='Death_Rate', group = 'State', color = 'State') , data = df_TX) +geom_point(color = 'grey', alpha = 0.2) +geom_smooth(method='lm', data=df_TX[df_TX['Year']<2007], color='blue') +
        geom_smooth(method='lm', data=df_TX[df_TX['Year']>=2007], color='red') + geom_vline(xintercept=2007, linetype = "dotted") + 
        xlim(2004, 2015)+
       labs(y='Death Rate by Overdose',title="Pre-Post in TX")
       )
...
fig_WA=(ggplot(aes(x='Year', y='Death_Rate', group = 'State', color = 'State') , data = df_WA) + geom_smooth(method='lm', data=df_WA[df_WA['Year']<2011], color='blue')+ 
        ...
        geom_point(color = 'grey', alpha = 0.2) + geom_vline(xintercept=2011, linetype = "dotted") + xlim(2004, 2015)+
         labs(y='Death Rate by Overdose', title="Pre-Post in WA"))
...
fig_WA =(ggplot(df_final, aes(x='Year',y ='Death_Rate', color='Policy_State'))
         +geom_point(df_final[(df_final['State'] =='WA')], color = 'grey', alpha = 0.2)
      ...
      + geom_vline(xintercept= 2011, linetype = 'dotted')+
        labs(y='Death Rate by Overdose',title='Diff-in-Diff Analysis - FL and Non-policy states (Policy Change in 2011)'))
...
fig_FL =(ggplot(df_final, aes(x='Year',y ='Death_Rate', color='Policy_State'))
          +geom_point(df_final[(df_final['State'] =='FL')], color = 'grey', alpha = 0.2)
      + geom_smooth(df_final[(df_final['State'] != 'FL')& (df_final['State'] !='TX') & 
      ...
      + geom_vline(xintercept= 2010, linetype = 'dotted')+
           labs(y='Death Rate by Overdose', title='Diff-in-Diff Analysis - FL and Non-policy states (Policy Change in 2010)'))
...
fig_TX =(ggplot(df_final, aes(x='Year',y ='Death_Rate', color='Policy_State'))
          +geom_point(df_final[(df_final['State'] =='TX')], color = 'grey', alpha = 0.2)
      + geom_smooth(df_final[(df_final['State'] != 'FL')& (df_final['State'] !='TX') & 
      ...
      + geom_vline(xintercept= 2007, linetype = 'dotted')+
           labs(y='Death Rate by Overdose', title='Diff-in-Diff Analysis - TX and Non-policy states (Policy Change in 2007)'))
